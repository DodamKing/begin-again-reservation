<template>
  <div class="bg-white m-3 p-5 rounded-xl shadow-lg border border-gray-100 relative">
    ...
    <div class="flex justify-between items-center mb-5">
      ...
      <div class="flex items-center space-x-2 bg-gray-50 rounded-lg p-1">
        <button 
          @click="changeMonth(-1)"
          class="p-2 text-gray-600 hover:bg-white hover:shadow-sm rounded-md transition-all duration-200"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
          </svg>
        </button>
        <span class="px-4 py-2 font-bold text-gray-800 text-sm min-w-24 text-center">
    ...
    <div class="grid grid-cols-7 gap-1 mb-3">
      <div v-for="day in ['일', '월', '화', '수', '목', '금', '토']" 
           :key="day" 
           class="text-center text-xs font-bold text-gray-600 py-3 bg-gray-50 rounded-lg"
    ...
    <div class="grid grid-cols-7 gap-1">
      <div 
        ...
        :key="date.dateString"
        class="min-h-14 p-2 transition-all duration-200 relative cursor-pointer rounded-lg"
        :class="{
      ...
      >
        <div class="text-xs font-medium text-center mb-1"
             :class="{
